...
| D021 | M001 | arch | 5-model ensemble | Grok 30%, Claude 20%, GPT-4o 20%, Gemini 15%, DeepSeek 15% | Matches reference doc: Grok primary forecaster (grok-3-fast), Claude news analyst (claude-sonnet-4), GPT-4o bull advocate (gpt-4o-mini), Gemini Flash bear advocate (gemini-2.5-flash), DeepSeek risk manager (deepseek-chat). Each API model falls back to heuristic when key missing. Configurable via /settings page. | Yes — model selection and weights adjustable |
| D022 | M001 | arch | Pause mechanism | PAUSE file + dashboard toggle | Softer than kill switch. Loop stays alive but skips cycles. Three ways to trigger: dashboard button, CLI --pause/--unpause, manual PAUSE file. | No |
